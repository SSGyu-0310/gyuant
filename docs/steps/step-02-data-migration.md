...
## 현재 상태 (코드 기준)
- CSV/JSON 산출물은 여전히 생성되며 일부 경로에서 폴백으로 사용됩니다.
- SQLite는 스마트머니 결과/문서형 데이터 저장에 사용됩니다.
- PostgreSQL은 일봉/수급 결과를 optional로 dual-write 합니다 (`USE_POSTGRES=true`).
...
- 목표: CSV/JSON 폴백을 제거하고 DB 단일 소스로 전환 (현재는 부분 적용)
