...
## 현재 프로그램의 상태 요약
- 데이터 수집은 FMP 기반이며, 옵션 체인 분석(`us_market/options_flow.py`)만 yfinance를 유지합니다.
- 파이프라인은 `us_market/update_all.py` 기준으로 단계별 스크립트가 정리되어 있습니다.
- 산출물은 CSV/JSON으로 생성되며(SQLite 문서/스마트머니 저장 포함), 일부 스크립트는 `USE_POSTGRES=true` 시 PostgreSQL에도 dual-write 합니다.
- Flask API는 PostgreSQL(우선) + CSV 폴백으로 시계열을 조회하고, 문서/스마트머니는 SQLite에서 읽습니다. 실시간 지표는 FMP quote를 사용합니다.
- 테스트는 DB 연결 체크 정도로 최소화되어 있습니다.
...
## 단점/리스크
- 스토리지 경로가 분산되어 있습니다(PostgreSQL + SQLite + CSV/JSON). 소스 일관성 드리프트 위험이 있습니다.
- CSV 폴백 경로가 남아 있어 운영 경로가 복잡합니다.
- 과거 시점 재현(백테스트)에 필요한 point-in-time 데이터 구조가 부족합니다.
- 옵션 체인 데이터는 yfinance 의존(공급자 리스크)입니다.

## 단기 목표에 맞는 실행 방향
### 1) FMP 안정화/정리
- FMP 클라이언트(`utils/fmp_client.py`)를 기본 경로로 유지하고, 옵션 체인만 예외로 남겨 둡니다.
- 산출물 스키마를 유지해 후속 로직 변경을 최소화하세요.
- CSV 폴백을 단계적으로 축소하고, 운영 소스를 명확히 고정하세요.
...
### 테스트 전략
- FMP 호출 경로를 모킹할 수 있는 테스트 헬퍼를 추가합니다.
