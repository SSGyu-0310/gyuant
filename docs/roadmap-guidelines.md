...
## 현재 프로그램의 상태 요약
- 데이터 수집이 yfinance 중심이며, `us_market/` 파이프라인에서 CSV/JSON 결과물을 생성하고 Flask API가 이를 읽는 구조입니다.
- 실시간/단기 가격 조회는 `flask_app.py`에서 yfinance를 직접 호출합니다.
- 파이프라인은 `us_market/update_all.py` 기준으로 단계별 스크립트가 정리되어 있습니다.
- 테스트는 일부 엔드포인트와 파일 계약(contract) 중심이며 yfinance mock에 의존합니다.
...
## 단점/리스크
- yfinance 의존성이 높아 안정성이 낮고, 데이터 스키마가 암묵적입니다.
...
- yfinance를 제거하기 전까지는 듀얼 소스 모드(환경 변수로 선택)로 운영하면 리스크가 줄어듭니다.
...
### 테스트 전략
- yfinance mock을 FMP mock으로 교체합니다.
